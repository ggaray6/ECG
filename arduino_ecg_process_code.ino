...
const float s_ecg[S_ECG_SIZE] = { /* Your ECG data here */ };
...
int ecg_buffer[ECG_BUFFER_SIZE];
int ecg_buffer_idx = 0;
  ...
  if (currentMicros - previousMicros >= PERIOD) {
    ...
    if(leads_are_on){           
      ...
      if(Serial.available() > 0) {
        int next_ecg_pt = Serial.parseInt();
        
        // Store ECG data point in buffer
        ecg_buffer[ecg_buffer_idx] = next_ecg_pt;
        ecg_buffer_idx = (ecg_buffer_idx + 1) % ECG_BUFFER_SIZE;
        
        // Send ECG data point over Bluetooth
        bluetoothSerial.println(next_ecg_pt);
        
        // Give next data point to the algorithm
        QRS_detected = detect(next_ecg_pt);

        // Send QRS detection status over Bluetooth
        bluetoothSerial.print("QRS Detected: ");
        bluetoothSerial.println(QRS_detected);
      }
      ...
      if(QRS_detected == true){
        ...
        bluetoothSerial.println("QRS Detected!");
      }
    }    
  }
}
...
// The detect function remains unchanged as it's for QRS detection.
boolean detect(float new_ecg_pt) {
  // Your QRS detection logic here
  ...
  return false; // Change this according to your detection logic
}
